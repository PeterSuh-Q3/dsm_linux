...
int syno_led_manufactory_write(struct file *filp,const char *buf,size_t count,loff_t *offp)
{
	...
	switch(val) {
		...
		case 1:
			if (syno_set_ethernet_led) {
				syno_set_ethernet_led(1, 0, 2); // turn on green led
			}
			break;
		case 2:
			break;
...
int router_exdisplay_handler(struct _SynoMsgPkt *pMsgPkt)
{
	...
	switch(pMsgPkt->usNum) {
		...
		case SYNO_LED_MASK_ALL:
			mask_all = 1;
			if (in_findme || (gMeshIsRE && !gMeshHeartBeatAlive)) {
				goto END;
			}
			phy_led_off();
			break;
		...
		case SYNO_LED_READY_TO_SETUP:
			...
			syno_set_led(SZ_LED_GREEN, "none", LED_STATUS_BLINK);
			syno_set_led(SZ_LED_RED, "none", LED_STATUS_OFF);
			break;
		case SYNO_SYS_RUN:
			if (in_findme || (gMeshIsRE && !gMeshHeartBeatAlive)) {
		...
		case SYNO_SYS_NO_SYSTEM:
			...
			syno_set_led(SZ_LED_RED, "none", LED_STATUS_BLINK);
			break;
		...
		case SYNO_LED_NETWORK_SETTING:
			...
			if (wifi &&  0 == strncmp(pMsgPkt->szMsg, "wps", pMsgPkt->usLen)) {
				syno_set_led(SZ_LED_WIFI, "none", LED_STATUS_FAST_BLINK);
				wps = 1;
			} else if (0 == strncmp(pMsgPkt->szMsg, "wifi", pMsgPkt->usLen)) {
				if (0 == mask_all && 0 == wps) {
					syno_set_led(SZ_LED_WIFI, "none", LED_STATUS_ON);
				}
				wifi = 1;
		...
		case SYNO_LED_NETWORK_SETTING_END:
			...
			if (0 == strncmp(pMsgPkt->szMsg, "wps", pMsgPkt->usLen)) {
				if (wifi) {
					...
					syno_set_led(SZ_LED_WIFI, "none", LED_STATUS_OFF);
				}
				wps = 0;
			} else if (0 == strncmp(pMsgPkt->szMsg, "wifi", pMsgPkt->usLen)) {
				if (0 == wps) {
		...
		case SYNO_LED_DISCONNECT:
			...
			syno_set_led(SZ_LED_GREEN, "none", LED_STATUS_SLOW_BLINK);
			syno_set_led(SZ_LED_RED, "none", LED_STATUS_SLOW_BLINK);
			break;
		default:
...
InitModuleType(struct synobios_ops *ops)
{
	...
	module_t type_3000axv10 = MODULE_T_RT3000axv10;
	module_t *pType = NULL;

	switch (model) {
		case MODEL_RT3000ax:
#ifdef MY_DEF_HERE
			if (!strncmp(gszSynoHWVersion, HW_RT3000ax, strlen(HW_RT3000ax))) {
				pType = &type_3000axv10;
			} else {
