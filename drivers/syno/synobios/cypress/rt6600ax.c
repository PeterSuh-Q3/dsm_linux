	...
	switch(val) {
		...
		case 1:
			if (syno_set_ethernet_led) {
				...
				syno_set_ethernet_led(1, 0, 2); // enable WAN green LED
...
InitModuleType(struct synobios_ops *ops)
{
	...
	module_t type_6600axv10 = MODULE_T_RT6600axv10;
	module_t *pType = NULL;

	switch (model) {
		case MODEL_RT6600ax:
#ifdef MY_DEF_HERE
			if (!strncmp(gszSynoHWVersion, HW_RT6600ax, strlen(HW_RT6600ax))) {
				pType = &type_6600axv10;
			} else {
